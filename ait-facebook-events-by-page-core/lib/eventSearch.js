...
EventSearch.prototype.search = function (options) {
    ...
    function getPages(url) {
        ...
        function getPagesPaging(first, url, after) {
            var tempurl;
            if (after !== "") {
            ...
            return rp.get(tempurl).then(function success(responseBody) {
                ...
                if (temp.paging && temp.paging.cursors && temp.paging.cursors.after) {
                    return getPagesPaging(false, url, temp.paging.cursors.after);
                } else {
                    return results;
                }
            })
        }
        return getPagesPaging(true, url, "");
    }
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!queryOptions.latitude || !queryOptions.longitude) {
            ...
            var idLimit = 50, //FB only allows 50 ids per /?ids= call
                ...
                venuesWithEvents = 0,
                eventsCount = 0,
                pageUrl = "https://graph.facebook.com/" + queryOptions.version + "/me/likes" +
                    "?limit=100" +
                    "&fields=id" +
            ...
            if (queryOptions.categories.length > 0) {
                pageUrl += "&categories=" + JSON.stringify(queryOptions.categories);
            }

            //Get places as specified
            getPages(pageUrl).then(function(responseBody) {
